...
@section('head')
    <style>
        ...
        .active{background:black; color:#fff !important}
    </style>
@endsection
    ...
    <section class="py-5 bg-light">
        ...
        <div class="row px-4 px-lg-5 py-lg-4 align-items-center">
            ...
            <nav aria-label="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{$title->name}}</li>
                </ol>
            </nav>
            </div>
        </div>
    ...
    <section class="py-5">
        ...
        <div class="row">
            ...
            <!-- <div class="py-2 px-4 bg-dark text-white mb-3"><strong class="small text-uppercase font-weight-bold">Fashion &amp; Acc</strong></div> -->
                <ul class="list-unstyled small text-muted pl-0 lg-4 font-weight-normal kategori">
                    @foreach($category as $row)
                        @if($row->parent != 0)
                            <li class="">
                                <a class="p-2 reset-anchor text-dark font-weight-bold d-block {{ Request::segment(2) == $row->id ? 'active' : ''}}" href="{{url('kategori/'.$row->id)}}">{{$row->name}}</a>
                            </li>
                        @endif
                    @endforeach
                </ul>
            ...
            <div class="row">
                @if(empty($product))
                    <div class="col-12">
                        <h5>Produk belum tersedia di kategori ini</h5>
                    </div>
                @else
